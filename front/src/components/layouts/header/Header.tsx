...
export const Header: React.FC = () => {
  ...
  const handleTestLogin = async () => {
    if (isTestLoggingIn) return;

    setIsLoggingIn(true);

    try {
      const response = await fetch('https://api.moyastudy.com/api/auth/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.message || '테스트 로그인에 실패했습니다.');
      }
    } catch (error) {
      console.error('테스트 로그인 요청 중 오류:', error);
      toast.error('테스트 로그인 요청 중 오류가 발생했습니다.');
    } finally {
              ...
              {loading ? (
                ...
                <div className="flex items-center space-x-2">
                  {/* <button
                    ...
                    className="flex items-center gap-2 px-3 py-1 text-sm font-medium text-white transition-all duration-200 bg-green-500 border rounded-lg hover:bg-green-600 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50">
                    {isTestLoggingIn ? (
                      <div className="w-4 h-4 border-2 border-white rounded-full animate-spin border-t-transparent" />
                    ) : (
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M12 2C13.1 2 14 2.9 14 4C14 5.1 13.1 6 12 6C10.9 6 10 5.1 10 4C10 2.9 10.9 2 12 2ZM21 9V7L15 1H5C3.89 1 3 1.89 3 3V21C3 22.11 3.89 23 5 23H19C20.11 23 21 22.11 21 21V9M19 9H14V4H19V9ZM7 19H17V17H7V19ZM7 15H17V13H7V15ZM7 11H14V9H7V11Z" />
                      </svg>
                    )}
                    {isTestLoggingIn ? '로그인 중...' : '테스트 로그인'}
                  </button> */}
          ...
          {!isLogin && (
            <div className="border-t border-gray-200 px-4 py-4">
              <button
                ...
                disabled={isTestLoggingIn}
                className="mb-2 flex w-full items-center justify-center gap-2 rounded-lg bg-green-500 px-3 py-2 text-sm font-medium text-white transition-all duration-200 hover:bg-green-600 disabled:cursor-not-allowed disabled:opacity-50">
                {isTestLoggingIn ? (
              ...
              <button
                ...
                disabled={isLoggingIn}
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-blue-500 px-3 py-2 text-sm font-medium text-white transition-all duration-200 hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50">
                {isLoggingIn ? (
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
                ) : (
