  ...
  instance.interceptors.request.use(
    async (config) => {
      ...
      if (token) {
        ...
        // 토큰이 만료된 경우 갱신 시도
        if (TokenStorage.isTokenExpired(token) && config.url !== OAUTH_ENDPOINTS.REFRESH) {
          try {
    ...
    (error) => {
      ...
      return Promise.reject(new AuthApiError('요청 설정 실패', undefined, undefined));
    }
  );
...
// Google 로그인
export const postGoogleAuth = async (authData: GoogleAuthResponse): Promise<ApiResponse<AuthResponseData>> => {
  try {
...
export const getUserInfo = async (): Promise<User> => {
  ...
  try {
    ...
    const response = await axiosInstance.get<ApiResponse<User>>(OAUTH_ENDPOINTS.USER_INFO);
