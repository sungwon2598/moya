...
import { studyApiService, UpdateCategoryRequest, CreateCategoryRequest } from '@/core/config/studyApiConfig';
...
const CategoryHeader = ({
    ...
    const buttonStyle = "px-3 py-1.5 bg-blue-500 text-white text-sm rounded-md shadow hover:bg-blue-600 transition-colors";
    const deleteButtonStyle = "px-3 py-1.5 bg-red-500 text-white text-sm rounded-md shadow hover:bg-red-600 transition-colors";
            ...
            <div className="flex items-center gap-3 flex-1">
                ...
                {showInput && (
                    <input
                        ...
                        className="px-3 py-1.5 border rounded-md flex-1 max-w-xs"
                        autoFocus
                    />
                )}
            </div>
                ...
                {selectedCategory && !isEditMode && !showInput && (
                    <>
                        <button
                            ...
                            className="px-3 py-1.5 bg-yellow-500 text-white text-sm rounded-md shadow hover:bg-yellow-600 transition-colors"
                        >
                            수정
                        </button>
                        {onDeleteCategory && (
...
const CategoryManagement = () => {
    ...
    const handleAddCategory = async () => {
        try {
            ...
            await studyApiService.createCategory(createRequest);
    ...
    const handleDeleteCategory = async (type: CategoryType, minorCategoryId?: number) => {
        try {
            ...
            await studyApiService.deleteCategory(categoryId);
    ...
    const handleEditCategory = (type: CategoryType) => {
        ...
        if (type === 'main') {
            ...
            const minorCategory = getSelectedSubCategorySubCategories().find(
                cat => cat.id === selectedMinorCategory
            );
    ...
    const handleSubmitEdit = async (type: CategoryType) => {
        try {
            ...
            await studyApiService.updateCategory(categoryId, updateRequest);
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto space-y-6">
            ...
            {/* 중분류 섹션 */}
            {selectedMainCategory && (
                <section className="bg-white shadow rounded-lg p-6 relative animate-[fadeIn_0.3s_ease-in-out]">
                    <CategoryHeader
                        title="중분류"
            ...
            {selectedSubCategory && (
                <section className="bg-white shadow rounded-lg p-6 relative animate-[fadeIn_0.3s_ease-in-out]">
                    <CategoryHeader
                        ...
                        selectedCategoryName={getSelectedSubCategorySubCategories().find(cat => cat.id === selectedMinorCategory)?.name}
                        isEditMode={isEditMode && activeInput === 'sub'}
                        onInputChange={setInputValue}
                        onToggleInput={() => toggleInput('sub')}
                        onAddCategory={handleAddCategory}
                        onDeleteCategory={selectedMinorCategory ? () => handleDeleteCategory('sub', selectedMinorCategory) : undefined}
